 ...
 *     "remark"          =>"优惠券领取记录",
 *     "author"          =>"",
 *     "create_time"     =>"2024-03-28 15:27:03",
 *     "version"         =>"1.0",
...
class ShopCouponUserInit extends Base
{
     ...
     * @param $order_num 订单号
     */
    public function use_coupon($coupon_id = 0, $order_num = 0)
    {
        $ShopCouponUserModel = new \initmodel\ShopCouponUserModel(); //优惠券领取记录  (ps:InitModel)
     ...
     * @param $coupon_id 优惠券id
     */
    public function cancel_use_coupon($coupon_id = 0)
    {
        $ShopCouponUserModel = new \initmodel\ShopCouponUserModel(); //优惠券领取记录  (ps:InitModel)
        $map                 = [];
        $map[]               = ['id', '=', $coupon_id];
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        $item['used_text']   = $this->used[$item['used']];//状态
