 ...
 *     "remark"          =>"资源评价",
 *     "author"          =>"",
 *     "create_time"     =>"2024-04-24 17:16:35",
 *     "version"         =>"1.0",
 *     "use"             => new \init\PlayOrderEvaluateInit();
 * )
 */
...
class PlayOrderEvaluateInit extends Base
{
    ...
    {
        $PlayOrderEvaluateInit  = new \init\PlayOrderEvaluateInit();//资源评价   (ps:InitController)
        $PlayOrderEvaluateModel = new \initmodel\PlayOrderEvaluateModel(); //资源评价  (ps:InitModel)
    }
    ...
    public function edit_post($params, $where = [])
    {
        ...
        if (!empty($params["id"])) {
            ...
            $result                = $PlayOrderEvaluateModel->strict(false)->update($params);
            if ($result) $result = $item["id"];
        } elseif (!empty($where)) {
            //传入where条件,根据条件更新数据
            $params["update_time"] = time();
            $result                = $PlayOrderEvaluateModel->where($where)->strict(false)->update($params);
            if ($result) $result = $item["id"];
        } else {
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $PlayOrderEvaluateModel->strict(false)->insert($params, true);
        }
    ...
    public function delete_post($id, $type = 1, $params = [])
    {
        ...
        if ($type == 1) $result = $PlayOrderEvaluateModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $PlayOrderEvaluateModel->destroy($id, true);//真实删除
    ...
    public function batch_post($id, $params = [])
    {
        ...
        $params["update_time"] = time();
        $result                = $PlayOrderEvaluateModel->where($where)->strict(false)->update($params);//修改状态
    ...
    public function list_order_post($list_order, $params = [])
    {
        ...
        foreach ($list_order as $k => $v) {
            $where   = [];
            $where[] = ["id", "=", $k];
            $result  = $PlayOrderEvaluateModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
        }
