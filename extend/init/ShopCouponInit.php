...
class ShopCouponInit extends Base
{
    ...
    public $InterfaceType = 'admin';//接口类型:admin=后台,api=前端
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        //处理转文字
        $item['type_text']   = $this->type[$item['type']];//优惠券类型
    ...
    public function edit_post($params, $where = [])
    {
        ...
        if ($params['indate']) $params['indate'] = strtotime($params['indate'] . '23:59:59');
        if ($params['start_time'] >= time()) $params['status'] = 1;
