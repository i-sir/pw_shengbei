...
class MemberInit extends Base
{
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        //提现总金额
        $map10                    = [];
        $map10[]                  = ['identity_type', '=', 'user'];
        $map10[]                  = ['user_id', '=', $item['id']];
        $item['total_withdrawal'] = $MemberWithdrawalModel->where($map10)->sum('price');

        //优惠券总数
        $map11                = [];
        $map11[]              = ['user_id', '=', $item['user_id']];
        $map11[]              = ['used', '=', 1];
        $item['total_coupon'] = $CouponUserModel->where($map11)->count();


        //团队人数
        $map12              = [];
        $map12[]            = ['pid', '=', $item['id']];
        $item['total_team'] = $MemberModel->where($map12)->count();
        ...
        if ($this->InterfaceType == 'api') {
            ...
            //统计订单金额
            $map20                = [];
            $map20[]              = ['user_id', '=', $item['id']];
            $map20[]              = ['status', 'in', [2, 20, 30, 40, 50, 60, 62, 65]];
            $item['total_amount'] = $PlayPackageOrderModel->where($map20)->sum('amount');
            $item['total_count']  = $PlayPackageOrderModel->where($map20)->count();
        }
     ...
     * @return mixed
     */
    public function get_list_join($where = [], $params = [])
    {
        $MemberModel = new \initmodel\MemberModel(); //会员管理  (ps:InitModel)
        $where[] = ['s.status', 'in', [2, 20, 30, 40, 50, 60, 62, 65]];

        $result = $MemberModel
            ->alias('m')
            ->leftJoin('play_package_order s', 'm.id=s.user_id')
            ->where($where)
            ->field('m.*')
            ->field('m.*, sum(s.amount) as total_amount,count(s.id) as total_count')
            ->group('m.id')
    ...
    public function edit_post($params)
    {
        ...
        if (!empty($params['id'])) {
            ...
            //用户基本信息生成
            $params['nickname'] = $this->get_member_wx_nickname();
            $params['avatar']   = cmf_config('app_logo');
