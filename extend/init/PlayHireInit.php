...
class PlayHireInit extends Base
{

    public $customer_type = [1 => '链接', 2 => '二维码'];//客服类型
    public $status        = [1 => '待租', 2 => '已锁定', 3 => '已出', 4 => '已结算', 5 => '取消'];//状态
    public $api_status        = [1 => '待租', 2 => '已锁定', 3 => '已锁定', 4 => '已锁定', 5 => '已锁定'];//状态
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        //处理转文字
        $item['customer_type_text'] = $this->customer_type[$item['customer_type']];//客服类型
        $item['status_text']        = $this->status[$item['status']];//状态

        //游戏名字
        $play_info = $PlayListModel->where('id', '=', $item['play_id'])->find();
        if ($play_info) {
            $item['play_name'] = $play_info['name'];
            $item['play_info'] = $play_info;
        }
        ...
        if ($this->InterfaceType == 'api') {
            ...
            $item['status_text']        = $this->api_status[$item['status']];//状态
    ...
    public function edit_post($params, $where = [])
    {
        ...
        if (!empty($params["id"])) {
            ...
            $result                = $PlayHireModel->strict(false)->update($params);
            if ($result) $result = $item["id"];
        } elseif (!empty($where)) {
            //传入where条件,根据条件更新数据
            $params["update_time"] = time();
            $result                = $PlayHireModel->where($where)->strict(false)->update($params);
            if ($result) $result = $item["id"];
        } else {
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $PlayHireModel->strict(false)->insert($params, true);
        }
    ...
    public function delete_post($id, $type = 1, $params = [])
    {
        ...
        if ($type == 1) $result = $PlayHireModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $PlayHireModel->destroy($id, true);//真实删除
    ...
    public function batch_post($id, $params = [])
    {
        ...
        $params["update_time"] = time();
        $result                = $PlayHireModel->where($where)->strict(false)->update($params);//修改状态
    ...
    public function list_order_post($list_order, $params = [])
    {
        ...
        foreach ($list_order as $k => $v) {
            $where   = [];
            $where[] = ["id", "=", $k];
            $result  = $PlayHireModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
        }
